  ...
  return (
    ...
    <section className="relative h-[100dvh] w-full overflow-hidden">
      ...
      <header ref={headerRef}
              className="absolute inset-x-0 top-0 z-20 flex flex-wrap items-center gap-x-2 gap-y-2 sm:gap-x-4
                         border-b border-regla bg-papel/85 px-3 py-2 backdrop-blur-[10px]">
        {/* On a narrow screen the title takes its own line and the picker and the
            buttons share the next one, the picker on the left and the buttons on the
            right. The two names at full length do not fit beside the buttons, so there
            the picker takes whatever is left over and lets them wrap onto two lines,
            which keeps them whole. From sm up it goes back to its natural width and
            everything sits on one line. */}
        <b className="w-full text-[1.04rem] font-bold sm:w-auto sm:text-[1.06rem]">
          {t.title}
        </b>

        <div className="flex min-w-0 flex-1 overflow-hidden rounded-lg border border-regla
          ...
          {VIEWS.map(({ key, path }, i) => (
            <button key={key} type="button" onClick={() => goTo(path)}
              className={`flex-1 px-[10px] py-[5px] text-[0.9rem] leading-tight transition-colors
